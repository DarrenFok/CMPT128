...
int main( int argc, char* argv[] )
{
  ...
  printf( "Image copied\n" );
  uint8_t* img_RegionSet = copy( img, imgwidth, imgheight );

  // Change color in a desired region to a desired color.
  printf( "Color changed in desired region\n");
  region_set(img_RegionSet, imgwidth, imgheight, 0, 0, 200, 200, 0);

  // draw the array as a grey-scale image in the window
  // This function returns after a user clicks or presses a button in the window
  draw_image_grey( img_RegionSet, imgwidth, imgheight );
